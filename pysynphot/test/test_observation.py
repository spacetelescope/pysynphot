...
class BinFluxCase(testutil.FPTestCase):
  def setUp(self):
    tmg = locations._refTable(
      os.path.join('mtab', 'OLD_FILES', 'u921351jm_tmg.fits'))
    tmc = locations._refTable(os.path.join(
        'mtab', 'OLD_FILES', 'ub31649mm_tmc.fits'))
...
class TestArithmetic(unittest.TestCase):
  def setUp(self):
    ...
    self.bigbox = S.Box(5000, 1000)

    self.obs = S.Observation(sp, bp, binset=self.bigbox.wave)
  ...
  def test_mult_scalar(self):
    ...
    self.assertEqual(tst.sample(4499.99), 0)
  ...
  def test_add_scalar(self):
    # is not allowed
    self.assertRaises(NotImplementedError, self.obs.__add__, 3)
  ...
  def test_add_spectrum(self):
    ...
    other = S.ArraySpectrum(wave=wv,
                            ...
                            waveunits='angstroms',
                            fluxunits='counts')
    self.assertRaises(NotImplementedError, self.obs.__add__, other)
