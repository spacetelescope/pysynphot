...
class TestUnitResponse(testutil.FPTestCase):
  def setUp(self):
    ...
    comptab = os.path.join('mtab', 'OLD_FILES', 'v8h1925fm_tmc.fits')
    thermtab = os.path.join('mtab', 'OLD_FILES','tae17277m_tmt.fits')

    S.setref(graphtable=locations._refTable(graphtab),
             comptable=locations._refTable(comptab),
             thermtable=locations._refTable(thermtab))
